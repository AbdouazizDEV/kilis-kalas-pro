...
@Component({
  ...
  templateUrl: './license-photo.page.html',
  styleUrls: ['./license-photo.page.scss'],
})
export class LicensePhotoPage {
  private readonly router = inject(Router);
  readonly registrationState = inject(DriverRegistrationStateService);

  @ViewChild('fileInput') fileInputRef?: ElementRef<HTMLInputElement>;

  readonly registrationStep = 3;
  readonly totalSteps = 5;

  get photoUrl(): string {
    return this.registrationState.draft$().licenseFront?.dataUrl ?? '';
  }

  get hasPhoto(): boolean {
    return !!this.registrationState.draft$().licenseFront;
  }
  ...
  async takePhoto(): Promise<void> {
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        if (photo.dataUrl) {
          this.savePhoto(photo.dataUrl, 'license-front.jpg', 'image/jpeg');
        }
        return;
      } catch {
